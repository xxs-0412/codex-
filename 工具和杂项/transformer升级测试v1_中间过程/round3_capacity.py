...
def save_life_error_bar(scan_df: pd.DataFrame, baseline_df: pd.DataFrame, out_path: Path) -> None:
    ...
    bars2 = ax.bar(x + width / 2, upgrade_vals, width=width, color=MODEL_COLORS["Transformer"], edgecolor="#ffffff", linewidth=1.0, label=f"Round3 (d={D_MODEL},ff={DIM_FF})")
    ...
    ax.set_title(f"Round3 (d_model={D_MODEL}, dim_ff={DIM_FF}) vs Baseline", fontsize=14, fontweight="bold", color=TEXT_COLOR, pad=12)
    ax.grid(axis="y", color=GRID_COLOR, linewidth=0.8, alpha=0.55)
    ax.set_axisbelow(True)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def save_summary_comparison(summary_df: pd.DataFrame, baseline_summary: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, (column, title, fmt) in zip(axes, metric_specs):
        labels = ["4.13v1 Baseline", f"Round3 (d={D_MODEL})"]
        vals = [
    ...
    fig.suptitle(f"Round3 (d_model={D_MODEL}, dim_ff={DIM_FF}) vs 4.13v1 Baseline", fontsize=14, fontweight="bold", color=TEXT_COLOR, y=0.98)
...
def main() -> None:
    ...
    print(f"Round: 3 (Model Capacity: d_model={D_MODEL}, dim_ff={DIM_FF})")
    ...
    print()
    print("=" * 60)
    print(f"Round 3 Complete: d_model={D_MODEL}, dim_ff={DIM_FF}")
    print("=" * 60)
    print(f"Mean life abs error: {float(scan_df['life_abs_error'].mean()):.0f} (baseline: 18673, best: 16809)")
