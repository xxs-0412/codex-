...
def save_life_error_bar(scan_df: pd.DataFrame, baseline_df: pd.DataFrame, out_path: Path) -> None:
    ...
    bars1 = ax.bar(x - width / 2, baseline_vals, width=width, color=BASELINE_COLOR, edgecolor="#ffffff", linewidth=1.0, label="4.13v1 Baseline (seq=6)", alpha=0.75)
    bars2 = ax.bar(x + width / 2, upgrade_vals, width=width, color=MODEL_COLORS["Transformer"], edgecolor="#ffffff", linewidth=1.0, label=f"Round2 (seq={SEQ_LEN})")
    ...
    ax.set_title(f"Round2 (seq_len={SEQ_LEN}) vs Baseline (seq_len=6)", fontsize=14, fontweight="bold", color=TEXT_COLOR, pad=12)
    ax.grid(axis="y", color=GRID_COLOR, linewidth=0.8, alpha=0.55)
    ax.set_axisbelow(True)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def save_summary_comparison(summary_df: pd.DataFrame, baseline_summary: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, (column, title, fmt) in zip(axes, metric_specs):
        labels = ["4.13v1 Baseline", f"Round2 (seq={SEQ_LEN})"]
        vals = [
    ...
    fig.suptitle(f"Round2 (seq_len={SEQ_LEN}) vs 4.13v1 Baseline", fontsize=14, fontweight="bold", color=TEXT_COLOR, y=0.98)
...
def main() -> None:
    ...
    print(f"Round: 2 (Sequence Length Optimization)")
    print(f"SEQ_LEN: {SEQ_LEN} (baseline: 6)")
    ...
    notes = [
        f"Transformer 升级测试 v1 — 第二轮：序列长度优化 (seq_len={SEQ_LEN})",
        "=" * 60,
        f"测试日期: 2026-04-13",
        f"设备: {DEVICE}",
        "",
        ...
        "",
        "训练配置（与基线相同）：",
        f"  epochs={EPOCHS}, lr={LEARNING_RATE}, weight_decay={WEIGHT_DECAY}",
        f"  dropout=0.0, 无验证集",
        "",
        "模型架构（未改动）：",
    ...
    print()
    print("=" * 60)
    print(f"Round 2 Complete: seq_len={SEQ_LEN}")
    print("=" * 60)
