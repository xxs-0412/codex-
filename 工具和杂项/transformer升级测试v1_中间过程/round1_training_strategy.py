...
def pick_validation_case(train_summary: pd.DataFrame) -> str:
    ...
    return str(sorted_df.iloc[mid_idx]["file_name"])


def train_model_with_validation(
    train_tables: dict[str, pd.DataFrame],
    val_tables: dict[str, pd.DataFrame],
) -> tuple[nn.Module, FeatureScaler, TargetScaler, dict]:
    all_train_seq, all_train_y = build_sequence_dataset(train_tables, SEQ_LEN)
    val_seq, val_y = build_sequence_dataset(val_tables, SEQ_LEN)

    seq_scaler = FeatureScaler().fit(all_train_seq.reshape(-1, all_train_seq.shape[-1]))
    target_scaler = TargetScaler().fit(all_train_y)
    train_x_scaled = to_tensor(scale_sequences(all_train_seq, seq_scaler))
    train_y_scaled = to_tensor(target_scaler.transform(all_train_y))
    raw_seq_t = to_tensor(all_train_seq)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        with torch.no_grad():
            ...
            val_loss = float(mse(val_pred, val_y_scaled).item())
        ...
        if epoch == 1 or epoch % 200 == 0:
            print(f"    epoch {epoch:4d} | train_loss={float(loss.item()):.6e} | val_loss={val_loss:.6e} | lr={optimizer.param_groups[0]['lr']:.2e}")
    ...
    return model, seq_scaler, target_scaler, training_info
...
def save_life_error_bar(scan_df: pd.DataFrame, baseline_df: pd.DataFrame, out_path: Path) -> None:
    ...
    bars2 = ax.bar(x + width / 2, upgrade_vals, width=width, color=MODEL_COLORS["Transformer"], edgecolor="#ffffff", linewidth=1.0, label="Round1 Upgrade")
    ...
    ax.set_title("Round1 vs Baseline: Life Error by Test Case", fontsize=14, fontweight="bold", color=TEXT_COLOR, pad=12)
    ax.grid(axis="y", color=GRID_COLOR, linewidth=0.8, alpha=0.55)
    ax.set_axisbelow(True)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def save_summary_comparison(summary_df: pd.DataFrame, baseline_summary: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, (column, title, fmt) in zip(axes, metric_specs):
        labels = ["4.13v1 Baseline", "Round1 Upgrade"]
        vals = [
    ...
    fig.suptitle("Round1 Training Strategy Upgrade vs 4.13v1 Baseline", fontsize=14, fontweight="bold", color=TEXT_COLOR, y=0.98)
...
def main() -> None:
    ...
    print(f"Round: 1 (Training Strategy Optimization)")
    print(f"Epochs: {EPOCHS} (with early stopping patience={ES_PATIENCE})")
    ...
    print(f"Validation: yes (1 case per fold)")
    ...
    baseline_path = SCRIPT_DIR.parent / "4.13v1基准测试" / "详细结果_各折各模型.csv"
    baseline_df = None
    ...
    if baseline_path.exists():
        ...
        baseline_summary_path = SCRIPT_DIR.parent / "4.13v1基准测试" / "汇总_各模型平均指标.csv"
    ...
    for fold_idx, test_row in eligible.iterrows():
        ...
        train_files = [str(f) for f in remaining["file_name"] if str(f) != val_file]
        ...
        run_count += 1
        print(f"[{run_count}/{total_folds}] test={test_source} | val={val_file} | train={len(train_files)} cases")
        ...
        model, seq_scaler, target_scaler, train_info = train_model_with_validation(train_tables, val_tables)
        print(f"    -> epochs_used={train_info['actual_epochs']} | best_val_loss={train_info['best_val_loss']:.6e} | final_lr={train_info['final_lr']:.2e}")
        ...
        [
            {
                ...
                "mean_actual_epochs": float(scan_df["actual_epochs"].mean()),
            }
        ]
    )
    ...
    notes = [
        "Transformer 升级测试 v1 — 第一轮：训练策略优化",
        "=" * 60,
        f"测试日期: 2026-04-13",
        f"设备: {DEVICE}",
        "",
        ...
        f"  epochs         1200 -> {EPOCHS} (with early stopping)",
        f"  验证集          无  -> 8训练+1验证+1测试",
        f"  LR调度          无  -> ReduceLROnPlateau(patience={LR_PATIENCE}, factor={LR_FACTOR})",
        f"  早停            无  -> patience={ES_PATIENCE}, min_delta={ES_MIN_DELTA}",
        "",
        "模型架构（未改动）：",
        ...
        f"单调性正则化: wear={MONO_LAMBDA_WEAR}, load={MONO_LAMBDA_LOAD}, clearance={MONO_LAMBDA_CLEARANCE}",
        "",
        f"达标工况数: {len(eligible)}",
        "",
        "4.13v1基线结果：",
        "  平均寿命绝对误差: 18673 转",
        "  压力MAE: 22.33 MPa",
    ...
    print()
    print("=" * 60)
    print("Round 1 Complete: Training Strategy Optimization")
    print("=" * 60)
    ...
    print(f"Mean actual epochs: {float(scan_df['actual_epochs'].mean()):.0f}")
    print()
