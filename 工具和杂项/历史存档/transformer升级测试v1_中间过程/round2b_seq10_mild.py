...
def save_life_error_bar(scan_df: pd.DataFrame, baseline_df: pd.DataFrame, out_path: Path) -> None:
    ...
    bars2 = ax.bar(x + width / 2, upgrade_vals, width=width, color=MODEL_COLORS["Transformer"], edgecolor="#ffffff", linewidth=1.0, label="Round2b (seq=10+mild train)")
    ...
    ax.set_title("Round2b (seq=10 + mild training) vs Baseline", fontsize=14, fontweight="bold", color=TEXT_COLOR, pad=12)
    ax.grid(axis="y", color=GRID_COLOR, linewidth=0.8, alpha=0.55)
    ax.set_axisbelow(True)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def save_summary_comparison(summary_df: pd.DataFrame, baseline_summary: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, (column, title, fmt) in zip(axes, metric_specs):
        labels = ["4.13v1 Baseline", "Round2b"]
        vals = [
    ...
    fig.suptitle("Round2b (seq=10 + mild training) vs 4.13v1 Baseline", fontsize=14, fontweight="bold", color=TEXT_COLOR, y=0.98)
...
def main() -> None:
    ...
    print(f"Round: 2b (seq=10 + mild training strategy)")
    ...
    for fold_idx, test_row in eligible.iterrows():
        ...
        run_count += 1
        print(f"[{run_count}/{total_folds}] test={test_source}")
    ...
    notes = [
        "Transformer 升级测试 v1 — 第二轮b：seq=10 + 温和训练策略",
        "=" * 60,
        f"测试日期: 2026-04-13",
        f"设备: {DEVICE}",
        "",
    ...
    print()
    print("=" * 60)
    print("Round 2b Complete: seq=10 + mild training")
    print("=" * 60)
    print(f"Mean life abs error: {float(scan_df['life_abs_error'].mean()):.0f} (baseline: 18673, seq10-only: 16809)")
