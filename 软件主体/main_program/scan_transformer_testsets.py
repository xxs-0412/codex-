...
def output_dir() -> Path:
    path = comparison_dir() / "Transformer跨测试集扫描"
...
def main() -> None:
    ...
    out_dir = output_dir()
    scan_df.to_csv(out_dir / "Transformer留一测试_详细误差.csv", index=False, encoding="utf-8-sig")
    ...
    brief_df.to_csv(out_dir / "Transformer留一测试_简表.csv", index=False, encoding="utf-8-sig")
    ...
    summary_stats.to_csv(out_dir / "Transformer留一测试_汇总.csv", index=False, encoding="utf-8-sig")
    ...
    (out_dir / "Transformer高误差原因分析.txt").write_text("\n".join(analysis_lines), encoding="utf-8")

    save_error_bar(scan_df, out_dir / "Transformer留一测试_寿命误差图.png")
