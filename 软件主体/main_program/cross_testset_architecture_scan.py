...
def output_dir() -> Path:
    path = bench.comparison_dir() / "跨测试集扫描"
...
def main() -> None:
    ...
    out_dir = output_dir()
    scan_df.to_csv(out_dir / "跨测试集扫描_各模型误差.csv", index=False)
    ...
    summary_stats.to_csv(out_dir / "跨测试集扫描_汇总.csv", index=False)

    save_grouped_life_error_plot(scan_df, out_dir / "跨测试集扫描_寿命误差分组图.png")
    ...
    (out_dir / "跨测试集扫描_说明.txt").write_text("\n".join(notes), encoding="utf-8")
