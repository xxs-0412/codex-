...
def processed_dir() -> Path:
    return processed_root() / "processed_dataset"


def comparison_dir() -> Path:
    path = processed_root() / "comparison_results"
    path.mkdir(parents=True, exist_ok=True)
    return path


def trained_model_dir() -> Path:
    path = processed_root() / "trained_model"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def processing_config_path() -> Path:
    return processed_root() / "processing_config.csv"
...
def save_model_bundle(
    ...
    model_path = trained_model_dir() / "stressnet_model_real.pt"
...
def main() -> None:
    ...
    curve_df = save_curve_comparison(
        comparison_dir() / "test_case_curve_comparison.png",
        true_curve_df,
        rollout_df,
        threshold_um,
        static_life,
    ...
    save_life_bar_chart(
        comparison_dir() / "test_case_life_bar_comparison.png",
        true_life_actual,
        static_life,
        recursive_life,
    )

    curve_df.to_csv(comparison_dir() / "test_case_curve_data.csv", index=False)
    rollout_df.to_csv(comparison_dir() / "test_case_recursive_rollout.csv", index=False)
    ...
    comparison_summary.to_csv(comparison_dir() / "test_case_life_comparison.csv", index=False)
    ...
    metrics_df.to_csv(comparison_dir() / "stress_metrics.csv", index=False)
    ...
    (processed_root() / "logs" / "training_log.txt").write_text("\n".join(log_lines), encoding="utf-8")
