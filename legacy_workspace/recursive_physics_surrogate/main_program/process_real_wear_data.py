...
def default_config_path() -> Path:
    return default_target_root() / "processing_config.csv"
...
def ensure_workspace(target_root: Path, config_path: Path) -> None:
    ...
    (target_root / "processed_dataset").mkdir(parents=True, exist_ok=True)
    (target_root / "comparison_results").mkdir(parents=True, exist_ok=True)
    (target_root / "trained_model").mkdir(parents=True, exist_ok=True)
...
def build_case_table(source_path: Path, config: dict) -> tuple[pd.DataFrame, dict]:
    ...
    summary_row = {
        "file_name": f"{source_path.stem.lower()}.csv",
        "source_file": source_path.name,
        "F": load_f,
        "D": diameter_d,
        "Cr": clearance_cr,
...
def main() -> None:
    ...
    processed_dir = args.target_root / "processed_dataset"
    ...
    if not source_files:
        raise FileNotFoundError(f"No supported source tables found in {source_dir}")
    ...
    for source_path in source_files:
        ...
        all_case_frames.append(case_df)

    summary_df = pd.DataFrame(summary_rows).sort_values("source_file").reset_index(drop=True)
    all_data_df = pd.concat(all_case_frames, ignore_index=True)

    summary_df.to_csv(processed_dir / "dataset_summary.csv", index=False)
    all_data_df.to_csv(processed_dir / "all_data.csv", index=False)

    log_lines = [
        "Real wear dataset processing complete.",
        f"Source directory: {source_dir}",
        f"Target directory: {processed_dir}",
        ...
        f"Processed cases: {len(summary_df)}",
    ]
    (args.target_root / "logs" / "processing_log.txt").write_text("\n".join(log_lines), encoding="utf-8")
