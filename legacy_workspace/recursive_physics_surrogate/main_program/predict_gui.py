...
class PredictionGUI:
    def __init__(self) -> None:
        ...
        self.save_sub_var = tk.StringVar(value="预测后自动保存 CSV / PNG")
    ...
    def _build_left_panel(self, parent: tk.Frame) -> None:
        ...
        input_card, input_body = self._card(
            parent,
            "工况输入",
            "输入载荷、几何尺寸、递推步长和磨损阈值。软件会自动保存预测结果。",
        )
        ...
        aux_card, aux_body = self._card(
            parent,
            "导出与状态",
            "预测后可以在这里另存结果，也可以查看当前参数文件和自动保存路径。",
        )
    ...
    def run_prediction(self) -> None:
        ...
        self.latest_plot_path = plot_path
        ...
        if reached and life_actual_cycles is not None:
            ...
            summary_text = (
                ...
                f"磨损阈值设定为 {normalized_request['wear_threshold_um']:.2f} um，程序已自动保存本次结果。"
            )
        else:
        ...
        self.save_value_var.set("Saved")
        self.save_sub_var.set("CSV / PNG 已自动保存")
