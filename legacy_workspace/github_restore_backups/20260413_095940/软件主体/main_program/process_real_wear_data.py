...
def canonical_case_token(path_or_name: Path | str) -> str:
    stem = Path(str(path_or_name)).stem.strip().lower()
...
def canonical_output_name(path_or_name: Path | str) -> str:
    stem = Path(str(path_or_name)).stem.strip()
    match = CASE_NAME_PATTERN.match(stem)
    if match:
        return f"试验{int(match.group(1))}.csv"
    return f"{stem}.csv"


def candidate_rank(path: Path, priorities: dict[str, int]) -> tuple[int, int, float]:
    stem = path.stem.strip()
    chinese_name_bonus = 1 if stem.startswith("试验") else 0
    return (priorities[path.suffix.lower()], chinese_name_bonus, path.stat().st_mtime)
...
def build_case_table(source_path: Path, config: dict) -> tuple[pd.DataFrame, dict]:
    ...
    summary_row = {
        "file_name": canonical_output_name(source_path),
        "source_file": source_path.name,
        "F": load_f,
        "D": diameter_d,
        "Cr": clearance_cr,
...
def main() -> None:
    ...
    for path in sorted(source_candidates, key=lambda item: item.name.lower()):
        key = canonical_case_token(path)
        current = selected_by_stem.get(key)
        if current is None:
            selected_by_stem[key] = path
            continue
        current_rank = candidate_rank(current, priorities)
        next_rank = candidate_rank(path, priorities)
