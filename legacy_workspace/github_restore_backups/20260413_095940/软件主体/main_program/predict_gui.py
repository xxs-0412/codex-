...
class PredictionGUI:
    def __init__(self) -> None:
        ...
        self.save_value_var = tk.StringVar(value="关闭")
        self.save_sub_var = tk.StringVar(value="当前不自动保存，可手动另存")
    ...
    def _build_left_panel(self, parent: tk.Frame) -> None:
        ...
        input_card, input_body = self._card(
            parent,
            "工况输入",
            "输入载荷、几何尺寸、递推步长和磨损阈值。预测完成后可按需手动另存结果。",
        )
        ...
        aux_card, aux_body = self._card(
            parent,
            "导出与状态",
            "预测后可以在这里另存结果，也可以查看当前参数文件和当前保存状态。",
        )
        ...
        tk.Label(
            export_inner,
            text="根目录 bat 启动方式保持不变，app_workspace 中的 bat 作为备份。",
            bg=self.colors["surface"],
            fg=self.colors["muted"],
            justify="left",
            wraplength=320,
    ...
    def _build_right_panel(self, parent: tk.Frame) -> None:
        ...
        metric_row.grid_rowconfigure(0, minsize=144)
        ...
        self._metric_card(metric_row, "自动保存", self.save_value_var, self.save_sub_var, self.colors["success"]).grid(
    ...
    def run_prediction(self) -> None:
        ...
        if reached and life_actual_cycles is not None:
            ...
            summary_text = (
                ...
                f"磨损阈值设定为 {normalized_request['wear_threshold_um']:.2f} um，本次结果尚未自动保存。"
            )
        else:
        ...
        self.save_value_var.set("关闭")
        self.save_sub_var.set("可在左侧导出页手动另存 CSV / PNG")
        ...
        self.status_var.set(
            ...
            "自动保存: 已关闭\n"
            "结果状态: 当前结果仅保存在本次界面会话中，可手动另存 CSV / PNG"
        )
...
def run_test_mode() -> None:
    ...
    csv_path, plot_path = save_prediction_artifacts(normalized_request, export_df, fig, stem=timestamp_stem(normalized_request))
    plt.close(fig)
