...
DEFAULT_MODEL_PATH = Path("storage") / "training_validation" / "current_enhanced_transformer_model.pt"
LEGACY_MODEL_PATH = Path("storage") / "training_validation" / "stressnet_model.pt"
...
class TransformerNet(nn.Module):
    def __init__(self, input_dim: int = 5, config: TransformerConfig | None = None) -> None:
        super().__init__()
...
def transform_raw_sequences(raw_sequences: np.ndarray, feature_columns: tuple[str, ...]) -> np.ndarray:
    ...
    actual_cycle = raw[..., 3]
    wear_depth = raw[..., 4]
    prev_stress = raw[..., 5] if raw.shape[-1] > 5 else np.zeros_like(actual_cycle)
    ...
    feature_map = {
        ...
        "log1p_actual_cycle": np.log1p(np.maximum(actual_cycle, 0.0)),
        "delta_cycle": _diff_with_zero(actual_cycle),
        "delta_wear": _diff_with_zero(wear_depth),
        "prev_stress": prev_stress,
    }
    return np.stack([feature_map[column] for column in feature_columns], axis=-1).astype(np.float32)


def transformer_config_from_bundle(bundle: dict, seq_len: int) -> TransformerConfig:
    raw_cfg = dict(bundle.get("transformer_config", {}))
    return TransformerConfig(
        seq_len=int(raw_cfg.get("seq_len", seq_len)),
        d_model=int(raw_cfg.get("d_model", 32)),
        nhead=int(raw_cfg.get("nhead", 4)),
        num_layers=int(raw_cfg.get("num_layers", 2)),
        dim_ff=int(raw_cfg.get("dim_ff", 64)),
        dropout=float(raw_cfg.get("dropout", 0.0)),
    )


def load_model(model_path: Path) -> tuple[nn.Module, Standardizer, Standardizer, dict]:
    warnings.filterwarnings("ignore", category=FutureWarning, message="You are using `torch.load` with `weights_only=False`.*")
    bundle = torch.load(model_path, map_location=DEVICE, weights_only=False)
    architecture = str(bundle.get("model_architecture", bundle.get("model_name", "Transformer")))
    seq_len = int(bundle.get("sequence_length", DEFAULT_SEQUENCE_LENGTH))
    feature_columns = tuple(bundle.get("feature_columns", bundle.get("feature_order", ["F", "D", "Cr", "actual_cycle", "wear_depth"])))

    if architecture == "FNN":
        model = FNNNet(len(feature_columns)).to(DEVICE)
    else:
        model = TransformerNet(len(feature_columns), transformer_config_from_bundle(bundle, seq_len)).to(DEVICE)
    ...
    metadata = {
        ...
        "model_name": str(bundle.get("model_name", "Transformer")),
        "feature_variant": str(bundle.get("feature_variant", bundle.get("feature_spec_name", "unknown"))),
        "shape_variant": str(bundle.get("shape_variant", "unknown")),
...
def predict_pressure_from_history(
    ...
    sequence_length = int(model_metadata.get("sequence_length", DEFAULT_SEQUENCE_LENGTH))
    feature_columns = tuple(model_metadata.get("feature_columns", ("F", "D", "Cr", "actual_cycle", "wear_depth")))
    x = make_sequence(history, sequence_length)[np.newaxis, :, :]
    x = transform_raw_sequences(x, feature_columns)
...
def rollout_prediction(
    ...
    while actual_cycle <= max_actual_cycles + EPS:
        pred_stress = predict_pressure_from_history(model, x_scaler, y_scaler, history, model_metadata)
        k_eff = REAL_WEAR_COEFF_MPA_INV
        rows.append(
            {
        ...
        delta_wear_mm = k_eff * pred_stress * delta_s_mm
            ...
            rows.append(
                {
                    ...
                    "mode": "recursive",
                    "k_eff": k_eff,
                    "delta_wear_um": delta_wear_mm * 1000.0,
                }
            )
        ...
        rows[-1]["delta_wear_um"] = delta_wear_mm * 1000.0
...
def export_prediction_dataframe(prediction_df: pd.DataFrame) -> pd.DataFrame:
    ...
    export_df = export_df[[col for col in columns if col in export_df.columns]]
    return export_df
...
def run_prediction_request(request: dict, model_override: Path | None = None):
    ...
    model, x_scaler, y_scaler, metadata = load_model(model_path)
