...
def main() -> None:
    ...
    metrics_df.to_csv(out_dir / "网络结构对比_指标汇总.csv", index=False)
    ...
    merged_curve.to_csv(out_dir / "网络结构对比_曲线数据.csv", index=False)

    save_mae_bar_chart(metrics_df, out_dir / "网络结构对比_误差柱状图.png")
    save_life_prediction_bar(metrics_df, true_life_actual, out_dir / "网络结构对比_寿命柱状图.png")
    ...
    (out_dir / "网络结构对比_说明.txt").write_text("\n".join(notes), encoding="utf-8")
    ...
    static_summary.to_csv(out_dir / "静态基线参考.csv", index=False)
